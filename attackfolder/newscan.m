...
inputRGBImage=imread('nnnlena.jpg');
imshow('nnnlena.jpg');
ll = imread('lena3.tif');
...
for k=1:wmSize
    extractedWM(k)=[(imageDCT1(indexMAt(k,1),indexMAt(k,2))/dctOrg(indexMAt(k,1),indexMAt(k,2))-1)*10];
...
%imwrite(uint8(finalWMImage), 'lena444_Final_WM.tif', 'tif');
...
threshold = max(SIMs)*0.9;

if (SIMs(200) >threshold)
